...
# ---------------------------
# RBFS es una alternativa a A* que usa menos memoria.
# Expande recursivamente el camino más prometedor hasta que su f-value
# excede el límite, luego retrocede y prueba alternativas.
# ---------------------------

def recursive_best_first_search(problem):
    """
    Punto de entrada para RBFS. Inicializa el nodo raíz y llama a rbfs.
    
    Args:
        problem: Instancia de Problem con estado inicial, objetivo y heurística
    
    Returns:
        tuple: (nodo_objetivo, f_value) si encuentra solución, (None, inf) si no
    """
...
def rbfs(problem, node, f_limit):
    """
    Función recursiva que implementa el algoritmo RBFS.
    
    Args:
        problem: Problema a resolver
        node: Nodo actual a explorar
        f_limit: Límite superior para f(n) - si se excede, retrocede
    
    Returns:
        tuple: (nodo_objetivo, nuevo_f_limit)
    
    Funcionamiento:
    1. Si el nodo es objetivo → retornar
    2. Expandir y obtener sucesores
    3. Si no hay sucesores → retornar fallo
    4. Mientras haya sucesores:
       - Tomar el mejor (menor f)
       - Si excede f_limit → retroceder
       - Recursivamente explorar el mejor con límite actualizado
       - Si encuentra solución → retornar
       - Si no, actualizar f del mejor y reintroducir en la cola
    """
    # Validación si estamos en el objetivo, en tal caso retornar nodo y f(n)
    if problem.is_goal(node.state):
        return node, node.f_value

    # Expansión del nodo, devuelve una lista de hijos
    successors = expand(node, problem)
    
    # Retornar cuando no hay sucesores
    if not successors:
        return None, float('inf')

    # Crear cola de prioridad a partir de los sucesores
    # Se comparan mediante el método __lt__ del nodo (por f_value)
    heapq.heapify(successors)

    while True:
        # Tomar el nodo con menor f-value (más prometedor)
        best = heapq.heappop(successors)
        
        # Caso en que el camino actual excede el límite
        # Retrocedemos y probamos otro camino
        if best.f_value > f_limit:
            return None, best.f_value
        
        # Guardamos el camino alternativo (segundo mejor)
        # Si no hay más sucesores, usamos infinito
        alternative = successors[0].f_value if successors else float('inf')
        
        # Llamamos recursivamente con el mejor nodo
        # El nuevo límite es el mínimo entre f_limit actual y la alternativa
        result, best.f_value = rbfs(problem, best, min(f_limit, alternative))
        
        # Si encontramos solución, retornar
        if result is not None:
            return result, best.f_value
        
        # Si no encontramos solución, reintroducir el nodo con f actualizado
        heapq.heappush(successors, best)


def rbfs_search(problem, max_depth=100):
    """
    Wrapper para RBFS que retorna el mismo formato que astar_search.
    Facilita la comparación entre ambos algoritmos.
    
    NOTA: RBFS puede ser más lento en casos complejos porque re-expande nodos
    cuando retrocede (trade-off entre memoria y tiempo).
    ...
    """
    result_node, f_value = recursive_best_first_search(problem)
    
    if result_node is None:
        return None, None, float('inf')
    
    # Reconstruir ruta de estados y acciones (igual que en A*)
    ruta_estados = []
    ruta_acciones = []
    current = result_node
    ...
    return ruta_estados, ruta_acciones, result_node.path_cost
